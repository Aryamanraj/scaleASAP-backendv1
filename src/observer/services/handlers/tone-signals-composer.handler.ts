...
import {
  buildToneSignalsPrompt,
  ToneSignalsEvidence,
} from '../../../ai/prompts/tone-signals.prompt';
import { ToneSignalsComposerInput } from '../../../common/interfaces/module-inputs.interface';

@Injectable()
export class ToneSignalsComposerHandler {
  constructor(
  ...
  async execute(run: ModuleRun): Promise<ResultWithError> {
    try {
      this.logger.info(
        `ToneSignalsComposerHandler.execute: Processing module run [moduleRunId=${run.ModuleRunID}, projectId=${run.ProjectID}, personId=${run.PersonID}]`,
      );

      const input: ToneSignalsComposerInput = run.InputConfigJson || {};
      ...
      const evidence: ToneSignalsEvidence = {
      ...
      const prompt = buildToneSignalsPrompt(evidence);
      ...
      const aiResponse = await this.aiService.run({
        ...
        taskType: AI_TASK.TONE_SIGNALS_INFERENCE,
      ...
      const parsed = await this.parseJsonResponseWithRetry(
        ...
        AI_TASK.TONE_SIGNALS_INFERENCE,
        500,
      );
      ...
      const claim = await Promisify<Claim>(
        this.claimWriterService.insertClaimAndSupersedePrevious({
          ProjectID: run.ProjectID,
          PersonID: run.PersonID,
          ClaimType: CLAIM_KEY.INSIGHTS_TONE_SIGNALS,
          GroupKey: 'single',
      ...
      this.logger.info(
        `ToneSignalsComposerHandler.execute: Created claim [claimId=${claim.ClaimID}]`,
      );
      ...
      this.logger.error(
        `ToneSignalsComposerHandler.execute: Error [error=${error.message}, moduleRunId=${run.ModuleRunID}, stack=${error.stack}]`,
      );
  ...
  private async parseJsonResponseWithRetry(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        this.logger.warn(
          `ToneSignalsComposerHandler.execute: Invalid JSON response, retrying with follow-up prompt [moduleRunId=${moduleRunId}, attempt=${attempt}, error=${lastError.message}]`,
        );

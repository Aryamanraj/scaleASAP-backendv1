...
import {
  buildLowQualityEngagementPrompt,
  LowQualityEngagementEvidence,
} from '../../../ai/prompts/low-quality-engagement.prompt';
import { LowQualityEngagementComposerInput } from '../../../common/interfaces/module-inputs.interface';

@Injectable()
export class LowQualityEngagementComposerHandler {
  constructor(
  ...
  async execute(run: ModuleRun): Promise<ResultWithError> {
    try {
      this.logger.info(
        `LowQualityEngagementComposerHandler.execute: Processing module run [moduleRunId=${run.ModuleRunID}, projectId=${run.ProjectID}, personId=${run.PersonID}]`,
      );

      const input: LowQualityEngagementComposerInput =
        run.InputConfigJson || {};
      ...
      const prompt = buildLowQualityEngagementPrompt(evidence);
      ...
      const aiResponse = await this.aiService.run({
        ...
        taskType: AI_TASK.LOW_QUALITY_ENGAGEMENT_EXTRACTION,
      ...
      const parsed = await this.parseJsonResponseWithRetry(
        ...
        AI_TASK.LOW_QUALITY_ENGAGEMENT_EXTRACTION,
        500,
      );
      ...
      const claim = await Promisify<Claim>(
        this.claimWriterService.insertClaimAndSupersedePrevious({
          ProjectID: run.ProjectID,
          PersonID: run.PersonID,
          ClaimType: CLAIM_KEY.INSIGHTS_LOW_QUALITY_ENGAGEMENT,
          GroupKey: 'single',
      ...
      this.logger.info(
        `LowQualityEngagementComposerHandler.execute: Created claim [claimId=${claim.ClaimID}]`,
      );
      ...
      this.logger.error(
        `LowQualityEngagementComposerHandler.execute: Error [error=${error.message}, moduleRunId=${run.ModuleRunID}, stack=${error.stack}]`,
      );
  ...
  private async parseJsonResponseWithRetry(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        this.logger.warn(
          `LowQualityEngagementComposerHandler.execute: Invalid JSON response, retrying with follow-up prompt [moduleRunId=${moduleRunId}, attempt=${attempt}, error=${lastError.message}]`,
        );

...
import {
  buildExternalSocialsPrompt,
  ExternalSocialsEvidence,
} from '../../../ai/prompts/external-socials.prompt';
import { ExternalSocialsComposerInput } from '../../../common/interfaces/module-inputs.interface';

@Injectable()
export class ExternalSocialsComposerHandler {
  constructor(
  ...
  async execute(run: ModuleRun): Promise<ResultWithError> {
    try {
      this.logger.info(
        `ExternalSocialsComposerHandler.execute: Processing module run [moduleRunId=${run.ModuleRunID}, projectId=${run.ProjectID}, personId=${run.PersonID}]`,
      );

      const input: ExternalSocialsComposerInput = run.InputConfigJson || {};
      ...
      const evidence: ExternalSocialsEvidence = {
      ...
      const prompt = buildExternalSocialsPrompt(evidence);
      ...
      const aiResponse = await this.aiService.run({
        ...
        taskType: AI_TASK.EXTERNAL_SOCIALS_EXTRACTION,
      ...
      const parsed = await this.parseJsonResponseWithRetry(
        ...
        AI_TASK.EXTERNAL_SOCIALS_EXTRACTION,
        500,
      );
      ...
      const claim = await Promisify<Claim>(
        this.claimWriterService.insertClaimAndSupersedePrevious({
          ProjectID: run.ProjectID,
          PersonID: run.PersonID,
          ClaimType: CLAIM_KEY.INSIGHTS_EXTERNAL_SOCIALS,
          GroupKey: 'single',
      ...
      this.logger.info(
        `ExternalSocialsComposerHandler.execute: Created claim [claimId=${claim.ClaimID}]`,
      );
      ...
      this.logger.error(
        `ExternalSocialsComposerHandler.execute: Error [error=${error.message}, moduleRunId=${run.ModuleRunID}, stack=${error.stack}]`,
      );
  ...
  private async parseJsonResponseWithRetry(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        this.logger.warn(
          `ExternalSocialsComposerHandler.execute: Invalid JSON response, retrying with follow-up prompt [moduleRunId=${moduleRunId}, attempt=${attempt}, error=${lastError.message}]`,
        );

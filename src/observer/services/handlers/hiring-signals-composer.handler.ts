...
import {
  buildHiringSignalsPrompt,
  HiringSignalsEvidence,
} from '../../../ai/prompts/hiring-signals.prompt';
import { HiringSignalsComposerInput } from '../../../common/interfaces/module-inputs.interface';

@Injectable()
export class HiringSignalsComposerHandler {
  constructor(
  ...
  async execute(run: ModuleRun): Promise<ResultWithError> {
    try {
      this.logger.info(
        `HiringSignalsComposerHandler.execute: Processing module run [moduleRunId=${run.ModuleRunID}, projectId=${run.ProjectID}, personId=${run.PersonID}]`,
      );

      const input: HiringSignalsComposerInput = run.InputConfigJson || {};
      ...
      const evidence: HiringSignalsEvidence = {
      ...
      const claim = await Promisify<Claim>(
        this.claimWriterService.insertClaimAndSupersedePrevious({
          ProjectID: run.ProjectID,
          PersonID: run.PersonID,
          ClaimType: CLAIM_KEY.INSIGHTS_HIRING_SIGNALS,
          GroupKey: 'single',
      ...
      this.logger.info(
        `HiringSignalsComposerHandler.execute: Created claim [claimId=${claim.ClaimID}]`,
      );
      ...
      this.logger.error(
        `HiringSignalsComposerHandler.execute: Error [error=${error.message}, moduleRunId=${run.ModuleRunID}, stack=${error.stack}]`,
      );

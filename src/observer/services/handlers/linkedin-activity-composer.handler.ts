...
import {
  buildLinkedinActivityPrompt,
  LinkedinActivityEvidence,
} from '../../../ai/prompts/linkedin-activity.prompt';
import { LinkedinActivityComposerInput } from '../../../common/interfaces/module-inputs.interface';

@Injectable()
export class LinkedinActivityComposerHandler {
  constructor(
  ...
  async execute(run: ModuleRun): Promise<ResultWithError> {
    try {
      this.logger.info(
        `LinkedinActivityComposerHandler.execute: Processing module run [moduleRunId=${run.ModuleRunID}, projectId=${run.ProjectID}, personId=${run.PersonID}]`,
      );

      const input: LinkedinActivityComposerInput = run.InputConfigJson || {};
      ...
      const evidence: LinkedinActivityEvidence = {
      ...
      const prompt = buildLinkedinActivityPrompt(evidence);
      ...
      const aiResponse = await this.aiService.run({
        ...
        taskType: AI_TASK.LINKEDIN_ACTIVITY_INFERENCE,
      ...
      const parsed = await this.parseJsonResponseWithRetry(
        ...
        AI_TASK.LINKEDIN_ACTIVITY_INFERENCE,
        400,
      );

      const confidence = Number(parsed?.linkedinActivity?.confidence ?? 0.5);
      ...
      const claim = await Promisify<Claim>(
        this.claimWriterService.insertClaimAndSupersedePrevious({
          ProjectID: run.ProjectID,
          PersonID: run.PersonID,
          ClaimType: CLAIM_KEY.INSIGHTS_LINKEDIN_ACTIVITY,
          GroupKey: 'single',
      ...
      this.logger.info(
        `LinkedinActivityComposerHandler.execute: Created claim [claimId=${claim.ClaimID}]`,
      );
      ...
      this.logger.error(
        `LinkedinActivityComposerHandler.execute: Error [error=${error.message}, moduleRunId=${run.ModuleRunID}, stack=${error.stack}]`,
      );
  ...
  private async parseJsonResponseWithRetry(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        this.logger.warn(
          `LinkedinActivityComposerHandler.execute: Invalid JSON response, retrying with follow-up prompt [moduleRunId=${moduleRunId}, attempt=${attempt}, error=${lastError.message}]`,
        );

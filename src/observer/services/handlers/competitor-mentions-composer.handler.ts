...
import {
  buildCompetitorMentionsPrompt,
  CompetitorMentionsEvidence,
} from '../../../ai/prompts/competitor-mentions.prompt';
import { CompetitorMentionsComposerInput } from '../../../common/interfaces/module-inputs.interface';

@Injectable()
export class CompetitorMentionsComposerHandler {
  constructor(
  ...
  async execute(run: ModuleRun): Promise<ResultWithError> {
    try {
      this.logger.info(
        `CompetitorMentionsComposerHandler.execute: Processing module run [moduleRunId=${run.ModuleRunID}, projectId=${run.ProjectID}, personId=${run.PersonID}]`,
      );

      const input: CompetitorMentionsComposerInput = run.InputConfigJson || {};
      ...
      const evidence: CompetitorMentionsEvidence = {
      ...
      const prompt = buildCompetitorMentionsPrompt(evidence);
      ...
      const aiResponse = await this.aiService.run({
        ...
        taskType: AI_TASK.COMPETITOR_MENTIONS_EXTRACTION,
      ...
      const parsed = await this.parseJsonResponseWithRetry(
        ...
        run.ModuleRunID,
        AI_MODEL.GPT_4O,
        AI_TASK.COMPETITOR_MENTIONS_EXTRACTION,
        400,
      );
      ...
      const claim = await Promisify<Claim>(
        this.claimWriterService.insertClaimAndSupersedePrevious({
          ProjectID: run.ProjectID,
          PersonID: run.PersonID,
          ClaimType: CLAIM_KEY.INSIGHTS_COMPETITOR_MENTIONS,
          GroupKey: 'single',
      ...
      this.logger.info(
        `CompetitorMentionsComposerHandler.execute: Created claim [claimId=${claim.ClaimID}]`,
      );
      ...
      this.logger.error(
        `CompetitorMentionsComposerHandler.execute: Error [error=${error.message}, moduleRunId=${run.ModuleRunID}, stack=${error.stack}]`,
      );
  ...
  private async parseJsonResponseWithRetry(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        this.logger.warn(
          `CompetitorMentionsComposerHandler.execute: Invalid JSON response, retrying with follow-up prompt [moduleRunId=${moduleRunId}, attempt=${attempt}, error=${lastError.message}]`,
        );

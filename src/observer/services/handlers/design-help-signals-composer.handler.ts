...
import {
  buildDesignHelpSignalsPrompt,
  DesignHelpSignalsEvidence,
} from '../../../ai/prompts/design-help-signals.prompt';
import { DesignHelpSignalsComposerInput } from '../../../common/interfaces/module-inputs.interface';

@Injectable()
export class DesignHelpSignalsComposerHandler {
  constructor(
  ...
  async execute(run: ModuleRun): Promise<ResultWithError> {
    try {
      this.logger.info(
        `DesignHelpSignalsComposerHandler.execute: Processing module run [moduleRunId=${run.ModuleRunID}, projectId=${run.ProjectID}, personId=${run.PersonID}]`,
      );

      const input: DesignHelpSignalsComposerInput = run.InputConfigJson || {};
      ...
      const evidence: DesignHelpSignalsEvidence = {
      ...
      const prompt = buildDesignHelpSignalsPrompt(evidence);
      ...
      const aiResponse = await this.aiService.run({
        ...
        taskType: AI_TASK.DESIGN_HELP_SIGNALS_EXTRACTION,
      ...
      const parsed = await this.parseJsonResponseWithRetry(
        ...
        AI_TASK.DESIGN_HELP_SIGNALS_EXTRACTION,
        500,
      );
      ...
      const claim = await Promisify<Claim>(
        this.claimWriterService.insertClaimAndSupersedePrevious({
          ProjectID: run.ProjectID,
          PersonID: run.PersonID,
          ClaimType: CLAIM_KEY.INSIGHTS_DESIGN_HELP_SIGNALS,
          GroupKey: 'single',
      ...
      this.logger.info(
        `DesignHelpSignalsComposerHandler.execute: Created claim [claimId=${claim.ClaimID}]`,
      );
      ...
      this.logger.error(
        `DesignHelpSignalsComposerHandler.execute: Error [error=${error.message}, moduleRunId=${run.ModuleRunID}, stack=${error.stack}]`,
      );
  ...
  private async parseJsonResponseWithRetry(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        this.logger.warn(
          `DesignHelpSignalsComposerHandler.execute: Invalid JSON response, retrying with follow-up prompt [moduleRunId=${moduleRunId}, attempt=${attempt}, error=${lastError.message}]`,
        );

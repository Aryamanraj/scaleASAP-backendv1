...
import {
  buildRevenueSignalPrompt,
  RevenueSignalEvidence,
} from '../../../ai/prompts/revenue-signal.prompt';
import { RevenueSignalComposerInput } from '../../../common/interfaces/module-inputs.interface';

@Injectable()
export class RevenueSignalComposerHandler {
  constructor(
  ...
  async execute(run: ModuleRun): Promise<ResultWithError> {
    try {
      this.logger.info(
        `RevenueSignalComposerHandler.execute: Processing module run [moduleRunId=${run.ModuleRunID}, projectId=${run.ProjectID}, personId=${run.PersonID}]`,
      );

      const input: RevenueSignalComposerInput = run.InputConfigJson || {};
      ...
      const evidence: RevenueSignalEvidence = {
      ...
      const prompt = buildRevenueSignalPrompt(evidence);
      ...
      const aiResponse = await this.aiService.run({
        ...
        taskType: AI_TASK.REVENUE_SIGNAL_INFERENCE,
      ...
      const parsed = await this.parseJsonResponseWithRetry(
        ...
        AI_TASK.REVENUE_SIGNAL_INFERENCE,
        400,
      );

      const confidence = Number(parsed?.revenueSignal?.confidence ?? 0.5);
      ...
      const claim = await Promisify<Claim>(
        this.claimWriterService.insertClaimAndSupersedePrevious({
          ProjectID: run.ProjectID,
          PersonID: run.PersonID,
          ClaimType: CLAIM_KEY.INSIGHTS_REVENUE_SIGNAL,
          GroupKey: 'single',
      ...
      this.logger.info(
        `RevenueSignalComposerHandler.execute: Created claim [claimId=${claim.ClaimID}]`,
      );
      ...
      this.logger.error(
        `RevenueSignalComposerHandler.execute: Error [error=${error.message}, moduleRunId=${run.ModuleRunID}, stack=${error.stack}]`,
      );
  ...
  private async parseJsonResponseWithRetry(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        this.logger.warn(
          `RevenueSignalComposerHandler.execute: Invalid JSON response, retrying with follow-up prompt [moduleRunId=${moduleRunId}, attempt=${attempt}, error=${lastError.message}]`,
        );
